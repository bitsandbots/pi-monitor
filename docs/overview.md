...
PiMonitor is a lightweight, self-hosted system monitoring and management console for Raspberry Pi and generic Linux devices. It provides a real-time web dashboard for viewing hardware metrics, controlling systemd services, inspecting running processes, and issuing power commands — all without requiring cloud connectivity.
...
- **Multi-node fleet management** — the optional Hub component aggregates any number of PiMonitor nodes into a single pane of glass.
...
|-----------|------------|-------------|------|
| **Node Agent** | `pi_monitor.py` | `8585` | Per-device monitor & control API |
| **Fleet Hub** | `hub/pi_monitor_hub.py` | `8686` | Multi-node aggregation dashboard |
